...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    sendEmail.welcomeJobSeekerEmail(user.email,user.name);
    ...
    res.status(201).json({ token }).redirect('/Login');
  ...
  try {
    if (email === process.env.ADMIN_EMAIL) {
      ...
      if (!data) {
        return res
          .status(400)
          .json({ message: "OTP expired or not found for admin" });
      }
      ...
      sendEmail.sendAdminLoginNotification(
    ...
    if (pendingData) {
      ...
      sendEmail.welcomeRecruiterEmail(employer.email,employer.name);
      return res.status(200).json({
        message: "OTP verified. Employer registered.",
        token,
        redirectedurl: "/index2",
      });
    } else if (loginData) {
      // 3. Login OTP Flow
...
const loginHandler = async (req, res) => {
  ...
  try {
    ...
    // ✅ Check for User (Job Seeker) login
    const user = await User.findOne({ email });
    if (user) {
      ...
      return res.status(200).json({
        ...
        token,
        role: user.role,
        redirectedurl: "http://localhost:3000/index2",
      });
    }

    // ✅ If email was not found in any collection
...
const resendOTP = async (req, res) => {
  ...
  try {
    ...
    console.error("REGISTER ERROR:", error); // 👈 Logs full error with stack
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
